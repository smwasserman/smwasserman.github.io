...
3. **The live site matched my files.** Before the store moved, `curl -i https://smwasserman.github.io` returned `HTTP/2 200` from `server: GitHub.com`, and the body matched my local `index.html` byte for byte. *What would have made it fail:* a stale deploy (the body wouldn't match), or the site still sitting in `src/` (a 404).
